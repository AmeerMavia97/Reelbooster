...
import { BsThreeDots, BsHandThumbsUp, BsHandThumbsUpFill } from "react-icons/bs";
...
function ActionButtons({ reel, setReels, user_id }: ActionButtonsProps) {
  ...
  const handleNativeShare = async () => {
    const shareUrl = `${window.location.origin}/reel/${reel.social_id}`;
    const shareData = {
      title: `${reel.User?.user_name ?? "yeteneksat.com"} on yeteneksat.com`,
      text: reel.social_desc || "Check out this video on yeteneksat.com!",
      url: shareUrl,
    };
    ...
    <div className="flex flex-col gap-5 place-items-center translate-y-[200px]">
      {reel.Music != null && (
        ...
        >
          <div
            ...
            style={{ clipPath: "path('M0,0 H32 V44 H20 Q16,16 0,20 Z')" }}
          >
            <Image
      ...
      >
        <div className="rounded-full sm:bg-dark/[0.04] bg-primary/[0.4] p-2">
          {reel.isLiked === true ? (
            <BsHandThumbsUpFill className="text-red-500" size={22} />
          ) : (
            <BsHandThumbsUp className="text-dark" size={22} />
          )}
        </div>
      ...
      <div className="relative">
        ...
        >
          <div className="rounded-full sm:bg-dark/[0.04] bg-primary/[0.4] p-2">
            <Image src="/home/comment.png" alt="comment" width={24} height={24} />
          </div>
          {commentAdded ? (
            <p className="text-xs text-dark">{reel?.total_comments + 1}</p>
          ) : (
        ...
        {activeCommentPostId === reel.social_id &&
          ...
          createPortal(
            ...
            >
              <div
                className="w-full h-[75vh] rounded-t-2xl overflow-hidden"
        ...
        {activeCommentPostId === reel.social_id && (
          <div className="hidden md:block absolute left-[60px] bottom-10 h-[500px] w-[400px] z-50">
            <ReelComment />
          </div>
        )}
      </div>
      ...
      >
        <div className="rounded-full sm:bg-dark/[0.04] bg-primary/[0.4] p-2">
          <RiShareForwardLine className="text-dark" size={24} />
        </div>
        {ReelSharedId === reel.social_id ? (
          <p className="text-xs text-dark">{reel.total_shares + 1}</p>
        ) : (
      ...
      >
        <div className="rounded-full sm:bg-dark/[0.04] bg-primary/[0.4] p-2">
          <Image
            src={reel.isSaved === true ? "/home/filled_bookmark.png" : "/home/bookmark.png"}
            alt="bookmark"
            width={23}
            height={23}
          />
      ...
      {/* Profile pic */}
      <button className="cursor-pointer" onClick={() => handleUserRoute(reel.user_id)}>
